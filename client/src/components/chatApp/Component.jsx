...
function ChatApp(props) {
  ...
  const api_openai_key = props.api_openai_key;
  const db_arn = props.db_arn;
  const db_name = props.db_name;
  const db_secret = props.db_secret;
  const movies_table = props.movies_table;
  ...
  function convertMarkdownLinksToHTML(message) {
    // Esta regex maneja paréntesis anidados correctamente
  ...
  const examplePrompts = (prompts) => {
    if (prompts.length == 0) {
      ...
      return (
        "Aquí hay algunos ejemplos de preguntas que me puedes hacer:\r\n\r\n" +
        prompts
          .map((prompt) => {
            return prompt + "\r\n";
          })
  ...
  async function handleRequest(input_text, base64_encode = true) {
    ...
    try {
      ...
      if (response && "choices" in response) {
        ...
        const llm_response = response.model;
        setLLM(llm_response);
      }
    } catch (error) {
