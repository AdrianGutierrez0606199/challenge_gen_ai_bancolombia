...
def lambda_handler(event, context):
    ...
    # this block is to be able to do testing with the lambda function in the management console
    try:
    ...
    try:
        ...
        return json.dumps({"choices": [{"message": {"content": f"[ERROR]Missing required parameter: {str(e)}"}}]})
    print("input json:",event["text"])
    ...
    try:
        ...
        return json.dumps({"choices": [{"message": {"content": f"[ERROR]Failed to get embedding: {str(e)}"}}]})
    ...
    try:
        ...
        return json.dumps({"choices": [{"message": {"content": f"[ERROR]Failed to execute database query: {str(e)}"}}]})
    ...
    try:
        ...
        return json.dumps({"choices": [{"message": {"content": f"[ERROR]Failed to process chat history or parsing the context: {str(e)}"}}]})
    ...
    try:
        ...
        return json.dumps({"choices": [{"message": {"content": f"[ERROR]Failed to get response from OpenAi API: {str(e)}"}}]})
